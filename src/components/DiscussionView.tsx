...
import { getDiscussion, users, addComment, toggleReactionData, addAttachment } from "@/data/dummyData";
...
export function DiscussionView() {
  ...
  const handleReact = (commentId: number, emoji: string) => {
    const result = toggleReactionData(commentId, 1, emoji);
    if (result.added && result.reaction) {
      storeAddReaction(result.reaction);
    } else {
      const existing = storeReactions.find(
        (r) => r.commentId === commentId && r.userId === 1 && r.emoji === emoji
      );
      if (existing) {
        storeRemoveReaction(existing.id);
      }
    }
    setLocalKey((k) => k + 1);
  };

  const handleReply = (content: string, mentions: number[] | null, attachments: { fileName: string; fileUrl: string; fileType: string; fileSize: number }[], parentId: number) => {
    ...
    // Scroll to bottom after reply is posted
  ...
  const handleSend = (content: string, mentions: number[] | null, attachments: { fileName: string; fileUrl: string; fileType: string; fileSize: number }[]) => {
    const newComment = addComment({
      candidateId: selectedCandidate!.id,
      agencyId: selectedAgency!.agencyId,
      userId: 1,
          ...
          <div className="space-y-5">
            {discussion.comments.map((item) => (
              <CommentCard
                ...
                onReact={handleReact}
                onReply={(content, mentions, attachments) => handleReply(content, mentions, attachments, item.comment.id)}
                users={users}
                currentUserId={1}
              />
            ))}
